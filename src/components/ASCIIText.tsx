...
const mathMap = function (n: number, start: number, stop: number, start2: number, stop2: number): number {
...
class AsciiFilter {
  ...
  constructor(renderer: any, options: AsciiFilterOptions = {}) {
    const { fontSize = 12, fontFamily = "'Courier New', monospace", charset = " .'\`^\",:;Il!i~+_-?][}{1)(|/tfjrxnuvczXYUJCLQ0OZmwqpdbkhao*#MW&8%B@$", invert = true } = options;
...
class CanvasTxt {
  ...
  constructor(txt: string, options: CanvasTxtOptions = {}) {
    const { fontSize = 200, fontFamily = 'Arial', color = '#fdf9f3' } = options;
    this.canvas = document.createElement('canvas');
    this.context = this.canvas.getContext('2d');
    this.txt = txt;
...
class CanvAscii {
  ...
  onMouseMove(evt: MouseEvent | TouchEvent) {
    const e = 'touches' in evt ? evt.touches[0] : (evt as MouseEvent);
